...
export function buildWfsUrl(base: string | URL, query: WfsQuery): URL {
  ...
  if (url.username || url.password || url.hash)
    invalid("WFS URLs cannot contain credentials or fragments.");
  for (const key of [...url.searchParams.keys()])
    if (OWNED.has(key.toLowerCase())) url.searchParams.delete(key);
  positive(query.limit, "limit");
  positive(query.startIndex, "startIndex", true);
  if (query.layer !== undefined && !NAME.test(query.layer))
    invalid("The WFS layer name is invalid.", { layer: query.layer });
  for (const property of query.properties ?? [])
    if (!NAME.test(property)) invalid("A WFS property name is invalid.", { property });
  url.searchParams.set("service", "WFS");
  url.searchParams.set("request", query.request);
  url.searchParams.set("version", query.version);
  if (query.request !== "GetCapabilities" && query.layer === undefined)
    invalid(`${query.request} requires a layer.`);
  if (query.layer !== undefined)
    url.searchParams.set(query.version === "2.0.0" ? "typeNames" : "typeName", query.layer);
  if (query.limit !== undefined)
    url.searchParams.set(query.version === "2.0.0" ? "count" : "maxFeatures", String(query.limit));
  if (query.startIndex !== undefined) url.searchParams.set("startIndex", String(query.startIndex));
  if ((query.properties?.length ?? 0) > 0)
    url.searchParams.set("propertyName", query.properties!.join(","));
  ...
  if (query.bbox !== undefined) {
    if (query.bbox.some((value) => !Number.isFinite(value)))
      invalid("WFS bbox coordinates must be finite.");
    url.searchParams.set(
      "bbox",
      [...query.bbox, ...(query.crs === undefined ? [] : [query.crs])].join(","),
    );
  }
