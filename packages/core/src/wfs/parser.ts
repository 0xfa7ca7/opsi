...
function parse(
  ...
  configure: (parser: SaxesParser<Options>) => void,
  ...
  if (Buffer.byteLength(text) > maxBytes)
    throw new OpsiError({
      ...
      context: { limit: maxBytes },
    });
  const parser = new SaxesParser<Options>({ xmlns: true });
  configure(parser);
  parser.on("doctype", () => {
    throw new Error("DTD declarations are forbidden");
  });
    ...
    (parser) => {
      parser.on("opentag", (tag: SaxesTagNS) => {
        ...
        if (stack.length === 1) {
          ...
          if (candidate === "2.0.0" || candidate === "1.1.0" || candidate === "1.0.0")
            version = candidate;
        }
      ...
      parser.on("closetag", (tag) => {
        ...
        if (current !== undefined) {
          ...
          else if (tag.local === "DefaultCRS" || tag.local === "DefaultSRS" || tag.local === "SRS")
            current.defaultCrs = value;
          else if (tag.local === "OtherCRS" || tag.local === "OtherSRS")
            current.otherCrs.push(value);
          else if (tag.local === "FeatureType") {
            if (current.name !== undefined)
              layers.push({
                name: current.name,
                ...(current.title === undefined ? {} : { title: current.title }),
                ...(current.defaultCrs === undefined ? {} : { defaultCrs: current.defaultCrs }),
                otherCrs: current.otherCrs,
              });
            current = undefined;
          }
        }
...
export function parseWfsSchema(
  xml: string | Uint8Array,
  layer: string,
  maxBytes = DEFAULT_MAX,
): readonly WfsField[] {
        ...
        if (tag.local === "element" && inSequence > 0) {
          ...
          if (name !== undefined && type !== undefined)
            fields.push({
              ...
              type,
              nullable:
                tag.attributes.minOccurs?.value === "0" ||
                tag.attributes.nillable?.value === "true",
            });
        }
      });
      parser.on("closetag", (tag) => {
...
export function parseWfsException(
  ...
  parse(
    ...
    (parser) => {
      parser.on("opentag", (tag: SaxesTagNS) => {
        if (tag.local === "Exception" || tag.local === "ServiceException")
          serviceCode = tag.attributes.exceptionCode?.value ?? tag.attributes.code?.value;
        text = "";
      });
      parser.on("text", (value) => {
        text += value;
      ...
      parser.on("closetag", (tag) => {
        if (tag.local === "ExceptionText" || tag.local === "ServiceException")
          exceptionText = text.trim().slice(0, 1024);
      });
    },
    maxBytes,
  );
  ...
  return new OpsiError({
    ...
    context: { ...(serviceCode === undefined ? {} : { serviceCode }) },
  });
}
