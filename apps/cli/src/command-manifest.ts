...
export const COMMAND_MANIFEST: readonly CommandManifestEntry[] = [
  ...
  leaf("service inspect", "Inspect a read-only WFS service", [argument("<resource>", "canonical WFS resource reference")], NETWORK_OPTIONS),
  leaf("service layers", "List WFS feature layers", [argument("<resource>", "canonical WFS resource reference")], NETWORK_OPTIONS),
  leaf("service schema", "Describe a WFS feature layer", [argument("<resource>", "canonical WFS resource reference")], [option("--layer <name>", "feature layer name", { mandatory: true }), ...NETWORK_OPTIONS]),
  leaf("service preview", "Preview bounded WFS features", [argument("<resource>", "canonical WFS resource reference")], [
    ...
    option("--property <name>", "selected field (repeatable or comma-separated)", { parser: "collect", defaultValue: [] }),
    option("--filter-eq <field=value>", "typed equality filter (repeatable)", { parser: "collect", defaultValue: [] }),
    option("--bbox <minx,miny,maxx,maxy>", "bounded spatial extent"), option("--crs <name>", "bbox coordinate reference system"), ...NETWORK_OPTIONS,
  ]),
  leaf("service count", "Count matching WFS features", [argument("<resource>", "canonical WFS resource reference")], [
    option("--layer <name>", "feature layer name", { mandatory: true }),
    option("--filter-eq <field=value>", "typed equality filter (repeatable)", { parser: "collect", defaultValue: [] }),
    option("--bbox <minx,miny,maxx,maxy>", "bounded spatial extent"), option("--crs <name>", "bbox coordinate reference system"), ...NETWORK_OPTIONS,
  ]),
  leaf("service export", "Export bounded WFS features to CSV", [argument("<resource>", "canonical WFS resource reference")], [
    option("--layer <name>", "feature layer name", { mandatory: true }), option("--output <path>", "destination CSV path", { mandatory: true }),
    option("--limit <rows>", "maximum exported rows", { parser: "positive" }), option("--start-index <number>", "zero-based feature offset", { parser: "nonnegative" }),
    option("--property <name>", "selected field (repeatable or comma-separated)", { parser: "collect", defaultValue: [] }),
    option("--filter-eq <field=value>", "typed equality filter (repeatable)", { parser: "collect", defaultValue: [] }),
    option("--bbox <minx,miny,maxx,maxy>", "bounded spatial extent"), option("--crs <name>", "bbox coordinate reference system"), option("--force", "replace an existing regular file"), ...NETWORK_OPTIONS,
  ]),
