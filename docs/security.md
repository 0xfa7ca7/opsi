...
`--allow-insecure-http`, `--allow-private-network`, `--force`, and `--debug` never persist as relaxed defaults. Destructive cache operations require `--yes`; non-TTY processes never hang for input. `OPSI_API_KEY` is environment-only, and config refuses secret-like keys. Errors omit causes/stacks by default; debug stacks pass terminal sanitation and redaction.
