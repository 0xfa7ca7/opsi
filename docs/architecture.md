...
Extensions implement `DataProvider` using provider-neutral branded IDs, canonical references, capabilities, and resolved-resource classification. New formats belong behind the data-engine contract and must support bounded detection, preview, validation, conversion, and tests where applicable. Input and output registries are deliberately separate: dense PC-Axis is an input adapter, while the writable format set remains CSV, TSV, JSON, NDJSON, XLSX, and Parquet.
...
A catalogue request flows CLI → `KlopsiClient` → `ProviderRegistry` → selected `DataProvider`; the provider validates upstream envelopes before mapping entities. A data request first resolves a local/canonical input, applies network policy where needed, stages content, invokes the selected handler, and returns domain-neutral rows/issues. Dense PC-Axis cubes are streamed into deterministic long-form rows: arbitrary STUB and HEADING dimensions become label columns, optional CODES become collision-safe sibling `__code` columns, and source symbols become `value: null` plus `value__symbol`. The Cartesian product is never materialized for preview or staging.

Query hashes resolved content (or reuses a verified download digest), detects its format, and keys an immutable DuckDB stage by content, selection, staging-contract version, format-specific bounds, and DuckDB compatibility version. PC-Axis limit values are part of that staging-contract identity so stages produced under different safety bounds cannot collide. A hit is linked or copied into a fresh invocation directory and opened read-only by the isolated worker; a miss stages once under a per-key build lock and publishes through the content-addressed cache. Output rendering is last so domain/core never knows terminal formats.
