...
Publish the public `opsi` package from the public `0xfa7ca7/opsi` GitHub repository through the existing tag-triggered GitHub Actions workflow. Keep npm trusted publishing, the protected `npm` environment, exact tested-tarball verification, and provenance. Do not add an npm token or a local publishing path.
...
After repository visibility becomes public, create the `npm` GitHub environment, protect tags matching `v*`, sign in to the npm CLI with two-factor authentication, and authorize `.github/workflows/release.yml` as the trusted publisher for package `opsi`. These account-level operations are intentionally separate from source changes.
