...
KLOPSI currently imports every query input into a fresh DuckDB database, runs one read-only query, and deletes the database. This design adds an automatic, content-addressed cache of immutable staged DuckDB databases. A repeated query over unchanged content reuses the staged database while preserving the existing CLI, query sandbox, output bounds, and temporary spill isolation.
...
- Keep `klopsi query <input> --sql <statement>` backward compatible and automatic by default.
- Preserve read-only query execution, disabled external access, disabled extension loading, and current resource limits.
- Reuse KLOPSI's content-addressed storage, atomic publication, cache locking, verification, and cache commands.
...
- Replacing KLOPSI's existing content cache or JSON metadata with a monolithic DuckDB database.
...
2. Stage the input into an invocation-local temporary database using only KLOPSI-owned SQL.
...
The core query service coordinates resolved source identity, derived-cache lookup, staging, publication, fallback, and result metadata. This keeps storage independent of DuckDB and keeps the data engine independent of KLOPSI cache policy.
...
- `klopsi cache prune` removes expired derived entries first, then least-recently-used entries until usage is within budget.
...
`klopsi cache info` adds separate raw and derived object counts and byte totals. Existing totals remain available for backward compatibility.

`klopsi cache list` identifies derived DuckDB records and reports size, creation time, last use, expiration, source digest, format, and sheet. It does not expose original paths or URLs.

`klopsi cache verify` composes storage's object-digest and strict-metadata verification with the data engine's structural verification of the `data` table. Structural verification opens the materialized database read-only with external access and extensions disabled; storage itself remains DuckDB-agnostic.

`klopsi cache prune` applies expiry and derived-only LRU enforcement in addition to existing unreferenced-object pruning.

`klopsi cache clear` keeps its existing interactive/non-interactive confirmation requirements and removes every cache category, including derived DuckDB artifacts.

## Security and privacy

- User SQL never reaches the writable staging connection.
- Cached databases are created from KLOPSI-owned `CREATE TABLE data AS ...` statements only.
...
- The public `klopsi query` invocation remains backward compatible.
