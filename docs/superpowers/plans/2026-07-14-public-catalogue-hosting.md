...
**Architecture:** The private scheduled workflow generates the existing bounded snapshot site, then pushes only that directory to `0xfa7ca7/opsi-catalogue` using a repository-scoped Ed25519 deploy key. GitHub Pages serves the public `gh-pages` branch, and the CLI's strict snapshot reader uses the resulting project Pages URL.
...
- Modify: `packages/catalogue-snapshot/test/remote.integration.test.ts`
...
- [ ] **Step 1: Write failing assertions** for `https://0xfa7ca7.github.io/opsi-catalogue/`, `git@github.com:0xfa7ca7/opsi-catalogue.git`, `gh-pages`, and `secrets.CATALOGUE_DEPLOY_KEY`, while retaining the per-job `@opsi/catalogue-snapshot...` build assertions.
...
- Produces: force-updated `gh-pages` branch at `git@github.com:0xfa7ca7/opsi-catalogue.git` and the page URL job output

- [ ] **Step 1: Replace** Pages artifact upload and OIDC deployment with a job that downloads the generated site artifact, writes the masked Ed25519 key with mode `0600`, pins GitHub's SSH host key, commits only generated files, and force-pushes `gh-pages`.
...
- [ ] **Step 3: Make verification target** `https://0xfa7ca7.github.io/opsi-catalogue/` and retry the existing strict verifier for bounded Pages propagation before failing.
- [ ] **Step 4: Run the Task 1 tests** and confirm the workflow contract passes.

### Task 3: Client endpoint and operator documentation

**Files:**
- Modify: `packages/catalogue-snapshot/src/remote.ts`
- Modify: `docs/catalogue-service.md`
- Modify: `docs/architecture.md`

**Interfaces:**
- Consumes: public Pages base URL
- Produces: `DEFAULT_CATALOGUE_BASE_URL = "https://0xfa7ca7.github.io/opsi-catalogue/"`

- [ ] **Step 1: Change the default URL** to the public data repository's project Pages URL.
...
**Files:**
- External: public repository `0xfa7ca7/opsi-catalogue`
...
- [ ] **Step 1: Create** the public data-only repository with no source checkout or reusable credential.
- [ ] **Step 2: Generate** a new Ed25519 key pair, add its public half as a write-enabled deploy key on `opsi-catalogue`, set its private half as `CATALOGUE_DEPLOY_KEY` on `opsi`, and securely remove the local private key.
