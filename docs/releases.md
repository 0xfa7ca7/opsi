...
After `opsi@0.0.1` is verified on npm, configure the trusted publisher while signed in to npm with 2FA, then immediately delete the bootstrap secret:

```sh
npm trust github opsi --repo 0xfa7ca7/opsi --file release.yml --env npm --allow-publish
